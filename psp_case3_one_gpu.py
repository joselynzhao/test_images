...
# --data=./output/car_dataset_3w_test/images --g_ckpt=car_model.pkl --outdir=../car_stylenrf_output/psp_case3/debug
@click.command()
...
def main(data, outdir, g_ckpt, e_ckpt,
    ...
    params+=list(G.synthesis.match2.parameters())
    ...
    for idx in pbar:
        ...
        rec_ws_1, c1= E(img_1,which_c='c1')
        rec_ws_2, c2= E(img_2,which_c='c1')
        # ws_avg = ws_avg.repeat(rec_ws_1.shape[0],1,1)  # zj 这个地方和之前得不一样，以前是不是也需要这样处理呢。
        rec_ws = rec_ws_1+rec_ws_2
        rec_ws +=ws_avg
        c = torch.cat((c1,c2),3)
        gen_img1 = G.get_final_output(styles=rec_ws_1, camera_matrices=camera1,c=c)
        gen_img2 = G.get_final_output(styles=rec_ws_1, camera_matrices=camera2,c=c)
        # rec_ws_2, c = E(gen_img2)
        # rec_ws_2 += ws_avg
        ...
        if i % 1000 == 0:
            ...
            snapshot_data1 = {}  # dict(training_set_kwargs=dict(training_set_kwargs))
            # snapshot_data2 = {}  # dict(training_set_kwargs=dict(training_set_kwargs))
            modules =[('G',G),('E',E)]
            for name,module in modules:
                if module is not None:
                    module = copy.deepcopy(module).eval().requires_grad_(False).cpu()
                snapshot_data1[name]=module
            # snapshot_data2['E'] = E
            # snapshot_data2['G'] = G  # 需要把G保存下来
            with open(snapshot_pkl, 'wb') as f:
                pickle.dump(snapshot_data1, f)
