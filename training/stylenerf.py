...
class NeRFBlock(nn.Module):
    ...
    def __init__(self, nerf_kwargs):
        ...
        elif self.positional_encoding == 'normal':  # 采用着这个方式
            dim_embed = D * self.n_freq_posenc * 2   # 60
    ...
    def forward(self, p_in, ray_d,H, z_shape=None,z_app=None,features=None,ws=None, shape=None, requires_grad=False,impl=None,camera_poses=None):
        ...
        # with torch.set_grad_enabled(self.training or self.use_sdf or requires_grad):
            ...
            option, p_in = self.forward_inputs(p_in, shape=shape, impl=impl) # p_in的batch就不等于4了。
            if self.tcnn_backend:
    ...
    def forward_nerf(self, option, p_in, H= None,ray_d=None, ws=None, z_shape=None, z_app=None, features=None,camera_poses=None):  # zj: h中要包含矩阵信息
        ...
        p = p.permute(0,3,1,2)    # BS x C x H x W  64 60 32 32
        ...
        if self.predict_rgb:  #this way
            ...
            feat_out = torch.cat([rgb, feat_out], 1)   # 256+3
    ...
    def get_evaluation_points(self, pixels_world=None, camera_world=None, di=None, p_i=None, no_reshape=False, transform=None):
        if p_i is None:
            ...
            p_i = camera_world.unsqueeze(-2).contiguous() + \
                di.unsqueeze(-1).contiguous() * ray_i.unsqueeze(-2).contiguous()  # 4，1024，16，3
...
class VolumeRenderer(object):
    ...
    def __init__(self, renderer_kwargs, camera_ray, input_encoding=None, **other_kwargs):
        ...
        self.embding1 = nn.Conv2d(3 +256, 3, 1, bias=False).cuda()
        self.embding2 = nn.Conv2d(3, 3, 1, bias=False).cuda()
        self.embding3 = nn.Conv2d(4 +256, 4, 1, bias=False).cuda()
        self.embding4 = nn.Conv2d(4, 4, 1, bias=False).cuda()
    ...
    def forward_rendering_background(self, H, output, bg_nerf, nerf_input_cams,latent_codes, styles_bg,features=None):
        ...
        feat, sigma_raw = bg_nerf(p_bg, r_bg,z_shape=z_shape_bg,z_app=z_app_bg,H = H, ws=styles_bg, shape=bg_shape,features=features)
        ...
        if (nerf_input_feats is not None) and \
            ...
            if (not not_render_background) and (not self.no_background):
                output = self.forward_rendering_background(  # 背景
                    H, output, bg_nerf, nerf_input_cams, latent_codes,styles_bg,features)
...
class Upsampler(object):
    ...
    def __init__(self, upsampler_kwargs, **other_kwargs):
        ...
        # self.match_c1 = MatchConv(128, 512).cuda()
        # self.match_c2 = MatchConv(256, 512).cuda()
        # self.match_c3 = MatchConv(512, 512).cuda()
    ...
    def forward_network(self, blocks, block_ws, x, img, target_res, alpha, skip_up=False, **block_kwargs):
        ...
        for index_l, (res, cur_ws) in enumerate(zip(self.block_resolutions, block_ws)):
            ...
            imgs += [img]
            # 在这个地方对image进行处理
            # if 'img_c' in block_kwargs and x.shape[-1]==64:#在这里对x进行match处理
...
class NeRFInput(Upsampler):
    ...
    def query_input_features(self, p_i, input_feats, p_shape, bound, grad_inputs=False):
        ...
        if input_feats[0] == 'tri_plane':
            ...
            p_f   = F.grid_sample(ms_v, p_yzx, mode='bilin**block_kwargs)ear', align_corners=False)
...
class NeRFSynthesisNetwork(torch.nn.Module):
    ...
    def forward(self, ws, views=None, source_views=None, features=None,**block_kwargs):  # 此时 block_kwargs 为空。
        #zj1010: 需要增加features，views ，source_views)
        # 之前的方案是把feature放到了block_kwargs里面。
        ...
        if "camera_matrices" not in block_kwargs:
            if 'camera_mode' in block_kwargs:
                ...
                if self.predict_camera:  # 随机选择一个mode
        ...
        if "latent_codes" not in block_kwargs:  #执行
            block_kwargs["latent_codes"] = self.get_latent_codes(batch_size, device=ws.device)  # latent_codes 到底是什么

        # deal with roll in cameras
        block_kwargs['theta'] = self.C.get_roll(ws, self.training, **block_kwargs)#返回是空
        ...
        with torch.autograd.profiler.record_function('nerf'):
            ...
            outputs = self.V.forward_volume_rendering(
                ...
                vol_pixels=vol_pixels, # batch， 1024，2
                nerf_input_feats=nerf_input_feats,  # none
                return_full=self.reg_full,  #False
                alpha=self.alpha,  #１
                features = features,  #将传入 c_feature
                camera_poses = self.get_camera(batch_size,device=ws.device,mode = views)[1], #取1只是外参，其实可以不用传。
                # zj : views 是cm, 就是最开始encoder计算出来的pose信息。
                **block_kwargs)  # zai block_kwarg里面放 C
        ...
        #     c_name,c,_,_,_ = block_kwargs['img_c']
        #     if c_name=='c1':
        #         x = self.match_c1(c,x)
        #     elif c_name =='c2':
        #         x = self.match_c2(c, x)
        ...
        #         x = self.match_c3(c, x)
            ...
            # 让x 和c 之间相加，在c是c2的时候，二者同纬度,w 256,32,32
            # x = x+c
...
class MatchConv(nn.Module):
    def __init__(
        ...
        self.emb1 = nn.Conv2d(in_channel1, 256, 1, bias=False)
        self.emb2 = nn.Conv2d(in_channel2, 256, 1, bias=False)
        # self.emb3 = nn.Conv2d(256, 256, 1, bias=False)
        # self.emb4 = nn.Conv2d(256, 256, 1, bias=False)
        self.emb3 = nn.Conv2d(in_channel1, in_channel1, 1, bias=False)
        self.emb4 = nn.Conv2d(in_channel1, in_channel2, 1, bias=False)
        self.att1 = nn.Conv2d(512, 128, 1, bias=False)
        self.att2 = nn.Conv2d(128, 1, 1, bias=False)
    ...
    def mydrop(self, x, iters):
        n,c,h,w = x.shape
        y = x.mean(1,keepdim=True).mean(2,keepdim=True).mean(3,keepdim=True)*0+1
        if iters<20000:
            return x*self.drop1(y)
        elif iters<40000:
            return x*self.drop2(y)
        elif iters<60000:
            return x*self.drop3(y)
        else:
            return x*self.drop4(y)

    def mynorm(self, x, tr=False):
        if tr:
            x = x.permute(0,2,1)
        x = F.instance_norm(x)
        if tr:
    ...
    def forward(self, Fe, Fg):  # p2和out
        ...
        # Fe = x
        # att = self.sig(self.att2(self.tanh(self.att1(Fe+Fg))))  # 直接去掉。
        return Fe+Fg
        # return x1+x2
