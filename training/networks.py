...
def modulated_conv2d(
    ...
    if x.dtype == torch.float16 and demodulate:
        ...
        weight = weight * (
                    1 / np.sqrt(weight_sizes) / weight.norm(float('inf'), dim=[1, 2, 3], keepdim=True))  # max_Ikk
    ...
    if demodulate and fused_modconv:
        w = w * dcoefs.reshape(batch_size, *rsizes,
                               1)  # [NOIkk]  (batch_size, out_channels, in_channels, kernel_size, kernel_size)
...
class MappingNetwork(torch.nn.Module):
    ...
    def forward(self, z=None, c=None, truncation_psi=1, truncation_cutoff=None, skip_w_avg_update=False, styles=None,
                **unused_kwargs):
...
class SynthesisLayer(torch.nn.Module):
    ...
    def forward(self, x, w, noise_mode='random', fused_modconv=True, gain=1, skip_up=False, input_noise=None,
                **unused_kwargs):
        ...
        if (up == 2) and (not use_default):
            ...
            if 'bilinear' in self.upsample_mode:
                ...
                x = F.interpolate(x, size=(resolution, resolution), mode='nearest') + torch.pixel_shuffle(
                    self.adapter(x), 2)
                ...
                x_pad[..., 1:-1, 1:-1] = x
                xl, xu, xd, xr = x_pad[..., 1:-1, :-2], x_pad[..., :-2, 1:-1], x_pad[..., 2:, 1:-1], x_pad[..., 1:-1,
                                                                                                     2:]
                ...
                x = torch.stack(
                    [x[..., self.coords[j, :, :, 0].long(), self.coords[j, :, :, 1].long()] for j in range(4)], 0)
...
class SynthesisLayer3(torch.nn.Module):
    ...
    def forward(self, x, w, noise_mode='random', force_fp32=False, **unused_kwargs):
        ...
        x = modulated_conv2d(x=x.to(dtype), weight=self.weight, styles=styles, padding=self.conv_kernel - 1, up=1,
                             fused_modconv=True)
...
class ToRGBLayer(torch.nn.Module):
    def __init__(self, in_channels, out_channels, w_dim=0, kernel_size=1, conv_clamp=None, channels_last=False,
                 mode='2d', **unused):
        super().__init__()
        self.conv_clamp = conv_clamp
        self.mode = mode
...
class SynthesisBlock(torch.nn.Module):
    ...
    def forward(self, x, img, ws, force_fp32=False, fused_modconv=None, add_on=None, block_noise=None,
                disable_rgb=False, **layer_kwargs):
        ...
        if img is not None:
            if img.size(-1) * 2 == x.size(-1):
                if (self.upsample_mode == 'bilinear_all') or (self.upsample_mode == 'bilinear_ada'):
                    ...
                    img = upfirdn2d.upsample2d(img,
                                               self.resample_filter)  # this is upsampling. Not sure about details and why they do this..
...
class Generator(torch.nn.Module):
    def __init__(self,
        ...
        self.mapping = None
        self.encoder = None
        # self.atten1 = Atten(512)
        # self.atten2 = Atten(512)
        #
...
@persistence.persistent_class
class super_Encoder(torch.nn.Module):
    def __init__(self,
                 img_resolution,
                 num_layers,
                 input_nc,
        ...
        # num_layers, input_nc, n_styles, mode='ir'
        self.psp_encoder = GradualStyleEncoder1(num_layers, input_nc, n_latents, mode)
        # G.img_resolution, G.mapping.num_ws, G.mapping.w_dim, add_dim=2
        self.stylenerf_encoder = Encoder(img_resolution, n_latents,w_dim, add_dim)

    def forward(self,img,which_c):
        ws1, c = self.psp_encoder(img,which_c=which_c)
        ws2, cm = self.stylenerf_encoder(img)
        ws = ws1 * self.balence +ws2*(1-self.balence)
        return ws,c,cm
...
class Encoder(torch.nn.Module):
    ...
    def forward(self, input):
        ...
        pcm = torch.sigmoid(pcm)
        return pws, pcm


@persistence.persistent_class
class view_relation_Encoder(torch.nn.Module):
    def __init__(self, c_channel, size, n_latents, w_dim=512, add_dim=0, **unused):
        super().__init__()
        ...
        self.n_latents = n_latents
        convs = [Conv2dLayer(c_channel, channels[size], 1)]
    ...
    def forward(self, input):
        ...
        out = self.projector(out)
        pws, pcm = out[:, :-2], out[:, -2:]
        pws = pws.view(len(input), self.c_channel)
        pcm = pcm.view(len(input), self.add_dim)
        pcm = torch.sigmoid(pcm)
        return pws, pcm
...
class Atten(nn.Module):
    def __init__(
        ...
        self.att1 = nn.Conv2d(in_channel, 128, 1, bias=False)
    ...
    def mydrop(self, x):
        n, c, h, w = x.shape
        x = x.reshape((n, c, 1, h * w)).permute(0, 3, 1, 2)
        x = self.drop(x).reshape(n, h, w, c).permute(0, 3, 1, 2)
        return x
    ...
    def forward(self, input1, input2):
        out = self._upsample_add(input1, input2)
        att = self.sig(self.att2(self.tanh(self.att1(out + input2))))
        return out * att + input2
