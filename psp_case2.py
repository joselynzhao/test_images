...
def ori_main(rank, word_size, data, outdir, g_ckpt, e_ckpt,
    ...
    for idx in pbar:
        ...
        rec_ws_1, c11,c12,c13= E(img_1)
        # ws_avg = ws_avg.repeat(rec_ws_1.shape[0],1,1)  # zj 这个地方和之前得不一样，以前是不是也需要这样处理呢。
        rec_ws_1 +=ws_avg
        gen_img1 = G.get_final_output(styles=rec_ws_1, camera_matrices=camera1,c=(c11,c12,c13))
        gen_img2 = G.get_final_output(styles=rec_ws_1, camera_matrices=camera2,c=(c11,c12,c13))
        rec_ws_2, c21,c22,c23 = E(gen_img2)
        rec_ws_2 += ws_avg
