...
#    with dnnlib.util.open_url('exp_encoder/checkpoints/network-snapshot-000010.pkl') as f:
#        resume_data = legacy.load_network_pkl(f)
#        E = copy.deepcopy(resume_data['E'].module).to(device)
        #misc.copy_params_and_buffers(resume_data['E'].module, E, require_all=False)
...
#    D = DDP(D, device_ids=[rank], output_device=rank, find_unused_parameters=True)
    E = DDP(E, device_ids=[rank], output_device=rank, find_unused_parameters=True)

    #E_optim = optim.Adam(E.parameters(), lr=lr*0.1, betas=(0.9, 0.99))
    E_optim = optim.Adam(E.parameters(), lr=lr*10, betas=(0.9, 0.99))
    ...
    for idx in pbar:
        ...
        c_samples = torch.from_numpy(np.random.rand(batch, 2)).to(torch.float32).to(device)
        #if truncation < 1.0:
        t = torch.rand(batch,1,1).to(torch.float32).to(device)
        w_samples = ws_avg + (w_samples - ws_avg) * (t*t*0.7+0.3)
...
#        loss_dict['loss_ws'] = F.smooth_l1_loss(rec_ws, w_samples).mean() * 10.0
        ...
        if i % 1000 == 0:
            ...
            with torch.no_grad():
                if rec_img is None:
                    rec_camera_matrices = G.synthesis.get_camera(batch, device, mode=rec_cm)
                    rec_img = G.get_final_output(styles=w_samples, camera_matrices=rec_camera_matrices)
