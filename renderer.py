...
class Renderer(object):
    ...
    def render_rotation_camera(self, *args, **kwargs):
        ...
        if 'img' not in kwargs:
            ...
            ws, _ = self.generator.encoder(kwargs['img'])  #[2,17,512]
        ...
        import  random
        seed = random.uniform(relatve_range_u[0], relatve_range_u[1])
        u_samples = []
        ...
        for step in tqdm.tqdm(range(n_steps)):
            # Set Camera
            u = u_samples[step]
            kwargs["camera_matrices"] = gen.get_camera(batch_size=batch_size, mode=[u, 0.5, 0.26], device=ws.device)
            cameras.append(gen.get_camera(batch_size=batch_size, mode=[u, 0.5, 0.26], device=ws.device))
#            kwargs["camera_matrices"] = gen.get_camera(batch_size=batch_size, mode=[0.4, u, 0.26], device=ws.device)
#            cameras.append(gen.get_camera(batch_size=batch_size, mode=[0.4, u, 0.26], device=ws.device))
#            kwargs["camera_matrices"] = gen.get_camera(batch_size=batch_size, mode=[0.5, 0.5, u], device=ws.device)
#            cameras.append(gen.get_camera(batch_size=batch_size, mode=[0.5, 0.5, u], device=ws.device))
            with torch.no_grad():
                out_i = gen(ws, **kwargs)
                if isinstance(out_i, dict):
                    imgs.append(out_i['img'][0:1])
        return imgs, cameras, ws
            # out.append(out_i)
        #zj
    ...
    def render_rotation_camera3(self, styles=None, *args, **kwargs): 
        ...
        for step in tqdm.tqdm(range(n_steps)):
            t = tspace[step]
            pitch = 1.5*0.2 * np.cos(t * 2 * np.pi) + np.pi/2 + 0.5
            yaw = 3*0.4 * np.sin(t * 2 * np.pi)
            u = (yaw - range_u[0]) / (range_u[1] - range_u[0])
            #u = (t*2*np.pi - range_u[0]) / (range_u[1] - range_u[0])
            v = (pitch - range_v[0]) / (range_v[1] - range_v[0])
            
            kwargs["camera_matrices"] = gen.get_camera(
                batch_size=batch_size, mode=[u, v*0.8-0.3, t*0.5+0.1], device=ws.device)
